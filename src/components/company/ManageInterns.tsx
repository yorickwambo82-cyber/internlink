...
export default function ManageInterns() {
  ...
  const handleComplete = async (applicationId: string) => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) {
        toast.success('Internship completed — certificate generated');
        setInterns((prev) => prev.filter((a) => a.id !== applicationId));
      } else {
        toast.error(data.error || 'Failed to complete internship');
      }
    } catch {
                ...
                <Card className="h-full hover:shadow-md transition-shadow">
                  ...
                  <CardContent className="space-y-4">
                    ...
                    <div className="flex gap-2 pt-1">
                      ...
                      <AlertDialog
                        open={completeDialogOpen && completeTargetId === app.id}
                        onOpenChange={(open) => {
                          setCompleteDialogOpen(open);
                          if (!open) setCompleteTargetId(null);
                        }}
                      >
                        <AlertDialogTrigger asChild>
                          <Button
                            size="sm"
                            variant="outline"
                            className="flex-1 text-xs h-8 gap-1 text-teal-600 hover:text-teal-700 hover:bg-teal-50"
                            disabled={completing === app.id}
                            onClick={() => setCompleteTargetId(app.id)}
                          >
                            {completing === app.id ? (
                        ...
                        <AlertDialogContent>
                          <AlertDialogHeader>
                            ...
                            <AlertDialogDescription>
                              Mark the internship of {app.student?.user?.name || 'this student'} as completed? A certificate will be generated automatically. This action cannot be undone.
                            </AlertDialogDescription>
                          </AlertDialogHeader>
                          <AlertDialogFooter>
